...
public class FUUKeyStream {
	public static void main(String[] args) throws Exception {
        ...
        AbstractDeserializationSchema<JSONObject> deserializationSchema = new MyDeserializationSchema();
        FlinkKafkaConsumer011<JSONObject> consumer = new FlinkKafkaConsumer011<JSONObject>(
                params.getRequired("read-topic"),
                deserializationSchema,
                params.getProperties());
        DataStream<JSONObject> mStream = env.addSource(consumer);
        
        CheckById cbd = new CheckById();
        DataStream<List<String>> resultIds = mStream.keyBy("id").map(cbd);

        resultIds.print();

        env.execute("FUUKeyStream");
	}
}